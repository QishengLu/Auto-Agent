...
def _validate_parquet_files(parquet_files: Union[str, List[str]]) -> List[str]:
    ...
    for file_path in parquet_files:
        ...
        if not path_obj.exists():
            # Try resolving relative to CWD if absolute path failed
        ...
        validated_files.append(str(path_obj))
    return validated_files


@register_plugin_tool("query_parquet_files")
...
@register_plugin_tool("get_schema")
def get_schema(parquet_file: str) -> str:
    ...
    path_obj = Path(parquet_file)
    if not path_obj.exists():
        # Try resolving relative to CWD if absolute path failed
        cwd_path = Path.cwd() / parquet_file.lstrip('/')
        if cwd_path.exists():
            path_obj = cwd_path
            parquet_file = str(path_obj)
        else:
            raise FileNotFoundError(
...
@register_plugin_tool("list_tables_in_directory")
def list_tables_in_directory(directory: str) -> str:
    ...
    if not dir_path.exists():
        # Try resolving relative to CWD if absolute path failed
        cwd_path = Path.cwd() / directory.lstrip('/')
        if cwd_path.exists():
            dir_path = cwd_path
        else:
            raise FileNotFoundError(
    ...
    # Use rglob to recursively find parquet files in subdirectories (like question_3)
